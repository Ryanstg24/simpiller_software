...
export function MedicationModal({ 
  ...
  useEffect(() => {
    if (mode === 'edit' && medication) {
      ...
      setTimePreferences({
        morning: getTimesArray(
          medication.patients?.morning_times || fallbackPatient?.morning_times,
          medication.patients?.morning_time || fallbackPatient?.morning_time,
          '06:00'
        ),
        afternoon: getTimesArray(
          medication.patients?.afternoon_times || fallbackPatient?.afternoon_times,
          medication.patients?.afternoon_time || fallbackPatient?.afternoon_time,
          '12:00'
        ),
        evening: getTimesArray(
          medication.patients?.evening_times || fallbackPatient?.evening_times,
          medication.patients?.evening_time || fallbackPatient?.evening_time,
          '18:00'
        ),
        bedtime: getTimesArray(
          medication.patients?.bedtime_times || fallbackPatient?.bedtime_times,
          medication.patients?.bedtime || fallbackPatient?.bedtime,
          '22:00'
        )
      });
    } else if (mode === 'add') {
      ...
      if (selectedPatientId) {
        ...
        if (patient) {
          ...
          setTimePreferences({
            morning: getTimesArray(patient.morning_times, patient.morning_time, '06:00'),
            afternoon: getTimesArray(patient.afternoon_times, patient.afternoon_time, '12:00'),
            evening: getTimesArray(patient.evening_times, patient.evening_time, '18:00'),
            bedtime: getTimesArray(patient.bedtime_times, patient.bedtime, '22:00')
          });
        }
      }
    }
